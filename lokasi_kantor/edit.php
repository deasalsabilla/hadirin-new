...
$stmtLokasi = $conn->prepare("SELECT * FROM lokasi_kantor WHERE id_lokasi = ?");
$stmtLokasi->bind_param("i", $id_lokasi);
$stmtLokasi->execute();
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($nama_lokasi) || empty($latitude) || empty($longitude) || $radius_meter <= 0) {
        ...
        $stmt = $conn->prepare("
            ...
            SET nama_lokasi = ?, latitude = ?, longitude = ?, radius_meter = ?
            WHERE id_lokasi = ?
        ");

        $stmt->bind_param("sddii", $nama_lokasi, $latitude, $longitude, $radius_meter, $id_lokasi);
...
<head>
    ...
    <title>Edit Lokasi Kantor - Sistem Absensi</title>
    <!-- Bootstrap 5 CSS -->
...
<body>
                        ...
                        <form method="POST" action="">
                            ...
                            <div class="form-group mb-3">
                                ...
                                <input type="number"
                                    ...
                                    class="form-control"
                                    min="1"
                                    value="<?php echo $data['radius_meter']; ?>"
                                    required>
                            </div>
    ...
    <script>
        ...
        // Marker lokasi yang sudah tersimpan
